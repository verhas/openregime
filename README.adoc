...
Later we may create a `pom.xml` to execute Jamal to automate that replacement process. The use of the `{{` and `}}`
separated placeholders is not Jamal specific and there can be many other tools to be used for the resolution
of the placeholders to the actual value. This is not a tool-lock-in. It just happens that Jamal was developed
by the same developer who has started this collection of the documents.
